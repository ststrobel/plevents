    ...
    app.put('/secure/users/:id/active/:active', async (req, res) => {
      ...
      if (user) {
        ...
        if (user.email === UserService.currentUser(req)) {
          res.status(409).send({ error: 'It is not possible to update himself' });
        } else {
          // now check if the user to change belongs to the organization of the logged-in admin
          if (user.tenantId === ((await UserService.currentTenant(req)) as Tenant).id) {
            user.active = req.params.active === 'true';
            user.save();
            res.status(200).send(user);
          } else {
            res.status(403).send({ error: 'You are not permitted to update users of a different organization' });
          }
        }
      } else {
    ...
    app.delete('/secure/users/:id', async (req, res) => {
      ...
      if (user) {
        ...
        if (user.email === UserService.currentUser(req)) {
          ...
          const usersOfTenant = (await User.findAll({ where: { tenantId: user.tenantId } })) as User[];
          if (usersOfTenant.length === 1) {
            res
              .status(409)
              .send({ error: 'It is not possible to delete your account, as it is the last within the tenant' });
          } else {
            user.destroy();
          ...
          if (user.tenantId === tenant.id) {
            ...
            Log.log(tenant, UserService.currentUser(req), logMessage);
            res.status(200).send({ message: 'Account deleted' });
          } else {
            res.status(403).send({ error: 'You are not permitted to update users of a different organization' });
          }
